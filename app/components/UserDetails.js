...
function UserDetails(user) {
  return <div>
    {Boolean(user.score) && <li className='list-group-item'><h3>Score: {user.score}</h3></li>}
    <li className='list-group-item'>
      <img src={user.info.avatar_url} className='img-rounded img-responsive'/></li>
    ...
    {user.info.blog && <li className='list-group-item'>Blog:
      <a href={user.info.blog}> {user.info.blog}</a></li>}
  </div>;
}
